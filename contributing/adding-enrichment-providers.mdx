...
You can list, update, and delete field extractions via the full [CRUD API](/api-reference/enrichment-field-extractions/list). Builtin provider extractions are **system-managed** — they can be toggled on/off but not deleted. When a custom provider is deleted, all its field extractions are automatically removed.
